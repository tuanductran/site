    ...
    <PageContainer title={title} intro={description}>
      ...
      <ul role="list" className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {filteredBooks.map((book: NotionBooks) => (
          <Link href={book.slug} key={book.id} className="group relative flex flex-col rounded-xl border border-zinc-200/50 bg-white p-6 dark:border-zinc-800/80 dark:bg-gradient-to-bl dark:bg-zinc-900 dark:from-zinc-950/5 dark:to-zinc-950/30 hover:bg-zinc-50 dark:hover:bg-zinc-800/50" title={book.title}>
